    ...
    <div class="container">
        <div class="bs-example" style="padding: 10px">
            <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                <button onclick="window.location='{{url('events/addEvents')}}'" class="btn btn-primary" type="button">Tambah Data</button>
            </div>
        </div>
        <div class="row">
                ...
                <table id="example" class="table table-striped nowrap" style="width:100%">
                    ...
                    <tbody>
                        @foreach ($dataEvents as $item)
                            <tr style="text-align: center">
                                <td><a href="{{route('events.edit',$item->id)}}">{{$item->deskripsi}}</a></td>
                                <td>{{$item->pic}}</td>
                                <td>{{$item->tanggal}}</td>
